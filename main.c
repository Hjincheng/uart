...
/*************************************************
 * thread_get_zigbee_addr
 **************************************************/
void* thread_get_zigbee_addr(void *arg)
{
    int fd = 0;
    char *uart_name = (char *)arg;
    unsigned char wbuff[] = "abcdefg";//{0xFE ,0x05 ,0x90 ,0x21 ,0x00 ,0x00,0x01,0xFF};
    unsigned char rbuff[20] = {0};
    int ret = 0;
    int i;
...
//	printf("%s:start\n", uart_name);
	ret = sys_uart_read(fd, rbuff, 7, 100);
        if(ret > 0){
...
int main(int argc, const char *argv[])
{
    ...
    if(argc != 3){
        printf("Usage: %s /dev/ttysWK2 /dev/ttysWK3\n", argv[0]);
        return -1;
    }
    
    printf("I am firely\n");

    if(pthread_create(&tid1, NULL, thread_get_zigbee_addr, (void *)argv[1])){
        printf("thread1 create err\n");
        return -1;
    }

    if(pthread_create(&tid2,NULL,thread_get_zigbee_addr, (void *)argv[2])){
